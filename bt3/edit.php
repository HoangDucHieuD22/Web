...
if (!isset($_GET['id'])) {
    echo "Không tìm thấy ID đoàn viên.";
    exit;
}

$id = intval($_GET['id']);

// Cập nhật dữ liệu nếu POST
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "UPDATE doanvien SET 
        ho_ten='$ho_ten', ma_sv='$ma_sv', nganh='$nganh', khoa='$khoa', ma_lop='$ma_lop',
        gioi_tinh='$gioi_tinh', ngay_sinh='$ngay_sinh', que_quan='$que_quan',
        email='$email', sdt='$sdt', ĐRL='$drl', GPA='$gpa'
        WHERE stt=$id";
...
<div class="container" style="padding-top: 50px;">
    <h2 class="text-center">Sửa Thông Tin Đoàn Viên</h2>
        ...
        <div class="row">
            <div class="col-md-6">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="ma_sv" class="form-control" required value="<?= htmlspecialchars($row['ma_sv']) ?>">
                </div>
                <div class="form-group">
                    <label>Ngành</label>
            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <input type="date" name="ngay_sinh" class="form-control" required value="<?= $row['ngay_sinh'] ?>">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="drl" class="form-control" required value="<?= htmlspecialchars($row['ĐRL']) ?>">
                </div>
                <div class="form-group">
                    <label>GPA</label>
                    <input type="text" name="gpa" class="form-control" required value="<?= htmlspecialchars($row['GPA']) ?>">
                </div>
            </div>
        </div>
...
document.querySelector("form").addEventListener("submit", function(event) {
    ...
    if (!phoneRegex.test(phone)) {
        errorMessages.push("Số điện thoại không hợp lệ (phải từ 9-11 chữ số).");
    }

    if (isNaN(gpa) || gpa < 0 || gpa > 4) {
        errorMessages.push("GPA phải là số trong khoảng 0 đến 4.");
    }

    if (isNaN(drl) || drl < 0 || drl > 100) {
        errorMessages.push("ĐRL phải là số từ 0 đến 100.");
    }
