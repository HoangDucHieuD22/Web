...
if (!isset($_SESSION['username'])) {
    header('Location: homepage.php'); // Chuyển hướng nếu chưa đăng nhập
    exit;
}

$ma_sv = $conn->real_escape_string($_SESSION['username']); // lấy mã sinh viên từ session

// Lấy thông tin cá nhân từ bảng doanvien theo ma_sv
$sql = "SELECT * FROM doanvien WHERE ma_sv = '$ma_sv'";
...
// Xử lý cập nhật thông tin cá nhân (nếu có gửi POST)
$success = $error = "";
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_info'])) {
    ...
    $update_sql = "UPDATE doanvien SET
        ho_ten = '$ho_ten',
        nganh = '$nganh',
        khoa = '$khoa',
        ma_lop = '$ma_lop',
    ...
    if ($conn->query($update_sql) === TRUE) {
        ...
        // Cập nhật lại $row để hiển thị thông tin mới
        $result = $conn->query($sql);
        $row = $result->fetch_assoc();
    } else {
...
// Xử lý đổi mật khẩu (nếu có gửi POST)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['change_password'])) {
    ...
    $acc_result = $conn->query("SELECT matkhau FROM taikhoan WHERE ma_sv = '$ma_sv'");
    if ($acc_result && $acc_result->num_rows > 0) {
        $acc = $acc_result->fetch_assoc();
        // Giả sử mật khẩu lưu theo dạng hash, dùng password_verify để so sánh
        if (password_verify($current_password, $acc['matkhau'])) {
            if ($new_password === $confirm_password) {
                $new_password_hash = password_hash($new_password, PASSWORD_DEFAULT);
                $update_pass_sql = "UPDATE taikhoan SET matkhau = '$new_password_hash' WHERE ma_sv = '$ma_sv'";
                if ($conn->query($update_pass_sql) === TRUE) {
    ...
    <form method="POST" action="">
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    <label>Họ tên</label>
                    <input type="text" name="ho_ten" class="form-control" required value="<?= htmlspecialchars($row['ho_ten']) ?>" />
                </div>
                <div class="form-group">
                    <label>Mã sinh viên</label>
                    <input type="text" class="form-control" readonly value="<?= htmlspecialchars($row['ma_sv']) ?>" />
                </div>
                <div class="form-group">
                    <label>Ngành</label>
                    <input type="text" name="nganh" class="form-control" required value="<?= htmlspecialchars($row['nganh']) ?>" />
                </div>
                <div class="form-group">
                    <label>Khoa</label>
                    <input type="text" name="khoa" class="form-control" required value="<?= htmlspecialchars($row['khoa']) ?>" />
                </div>
                <div class="form-group">
                    <label>Lớp</label>
                    <input type="text" name="ma_lop" class="form-control" required value="<?= htmlspecialchars($row['ma_lop']) ?>" />
                </div>
                <div class="form-group">
            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <input type="date" name="ngay_sinh" class="form-control" required value="<?= htmlspecialchars($row['ngay_sinh']) ?>" />
                </div>
                <div class="form-group">
                    <label>Quê quán</label>
                    <input type="text" name="que_quan" class="form-control" required value="<?= htmlspecialchars($row['que_quan']) ?>" />
                </div>
                <div class="form-group">
                    <label>Email</label>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="sdt" class="form-control" required value="<?= htmlspecialchars($row['sdt']) ?>" />
                </div>
                <div class="form-group">
                    <label>Điểm rèn luyện (ĐRL)</label>
                    <input type="text" class="form-control" readonly value="<?= htmlspecialchars($row['ĐRL']) ?>" />
                </div>
                <div class="form-group">
                    <label>GPA</label>
                    <input type="text" class="form-control" readonly value="<?= htmlspecialchars($row['GPA']) ?>" />
                </div>
            </div>
        </div>
